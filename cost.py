...
file=file.read()
df_new=pd.DataFrame(open("infection.csv"))
def create_smth(filename):
  df = pd.read_excel(filename)
  return df
df_new=create_smth("infection.csv")
...
# Driver code
  ...
  for i in range(1,len(the_matrix)):
    print(type(the_matrix[i]))

    G.addEdge(the_matrix[i][1], the_matrix[i][2])
  G.visualize()
